...
def plot_result(epoch, num_plot, fig_dir, pred_batch, X_batch, Y_batch=None):
  plt.figure()
  for i in range(min(num_plot, len(pred_batch))):
    plt.clf()
    gs = gridspec.GridSpec(5, 1)
    plt.subplot(gs[:4, :])
    # plt.imshow(X_batch[i,:,:,0])
    plt.pcolormesh(X_batch[i,:,:,0], cmap='Greys')
    plt.gca().set_xticklabels([])
    # plt.colorbar()
    plt.subplot(gs[4, :])
    plt.plot(pred_batch[i,0,:,1], label='\hat{y}')
    plt.ylim([-0.1, 1.1])
    plt.autoscale(enable=True, axis='x', tight=True)
    if Y_batch is not None:
      plt.plot(Y_batch[i,0,:,1], label='y')
      plt.ylim([-0.1, 1.1])
    plt.savefig(os.path.join(fig_dir, "{:03d}_{:03d}".format(epoch, i)))
  plt.close()
  return 0
