...
import glob
files = [f for f in glob.glob('/data/beroza/kaiwenw/Geysers/FilmDataDetection/test_pred_syn_0731234255mod99/results/' + "*.npz", recursive=True)]
file = open('/data/beroza/kaiwenw/Geysers/FilmDataDetection/test_pred_syn_0731234255mod99/picksfile.txt','a') 
for ifile in range(len(files)):
    preds=np.load('/data/beroza/kaiwenw/Geysers/FilmDataDetection/test_pred_syn_0731234255mod99/results/'+files[ifile].split('/')[-1][0:-4]+'.npz')
