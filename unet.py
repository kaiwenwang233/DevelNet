...
class UNet:
  ...
  def add_prediction_op(self):
    ...
    with tf.variable_scope("Output"):
      M_out = tf.get_variable("output_transform", [self.X_shape[0], self.Y_shape[0]])
      # net = tf.transpose(net, perm=[0, 2, 3, 1])
      # net_shape = net.get_shape().as_list()[:3]
      # net = tf.reshape(net, [net_shape[0]*net_shape[1]*net_shape[2], self.X_shape[0]])
      # net = tf.matmul(net, M_out)
      # net = tf.reshape(net, [net_shape[0], net_shape[1], net_shape[2], self.Y_shape[0]])
      # net = tf.transpose(net, perm=[0, 3, 1, 2])
      net = tf.einsum('aibc, id->adbc', net, M_out)
